...
public:
  ...
  static std::shared_ptr<NewMesh> cone(int divisions);
  static std::shared_ptr<NewMesh> cylinder(int divisions);
  static std::shared_ptr<NewMesh> line(glm::vec3 start, glm::vec3 end);
  static std::shared_ptr<NewMesh> grid(unsigned int divisions);
  static std::shared_ptr<NewMesh> circle(unsigned int divisions);
private:
};
